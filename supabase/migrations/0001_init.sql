...
begin
  if p_guest_id is not null then
    ...
    if p_photo_url is not null then
      update public.guests set photo_url = p_photo_url where guests.id = v_id;
    end if;
    ...
    if v_id is not null then
      ...
      insert into public.guests(name, company, gender, title, role, photo_url, status)
        values (btrim(p_name), nullif(btrim(coalesce(p_company, '')), ''),
                coalesce(p_gender, 'unknown'), nullif(btrim(coalesce(p_title, '')), ''),
                nullif(btrim(coalesce(p_role, '')), ''), p_photo_url, 'registered')
